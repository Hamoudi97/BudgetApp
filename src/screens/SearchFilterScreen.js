...
export default function SearchFilterScreen() {
  ...
  useEffect(() => {
    setData(expenses);
    setCategories(getUniqueCategories(expenses));
  }, [expenses]);
  ...
  const showSystemAlert = () => {
    ...
    currentBudgets.forEach((budget) => {
      ...
      if (spent > budget.amount) {
        overBudget.push(
          `${budget.category}: $${(spent - budget.amount).toFixed(2)} over`
        );
      }
    });
    ...
    if (overBudget.length > 0) {
      Alert.alert(
        ...
        [
          { text: "View Budget", style: "default" },
          { text: "Dismiss", style: "cancel" },
        ]
      );
    } else {
      Alert.alert("✅ Good News!", "You're within budget for all categories!");
    }
  };
  ...
  const renderItem = ({ item }) => (
    <Animated.View style={[styles.item, { opacity: fadeAnim }]}>
      ...
      <Text
        style={[
          styles.itemAmount,
          { color: item.amount > 50 ? "#FF7043" : "#81C784" },
        ]}
      >
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Search */}
      <Animated.View
        style={[styles.searchBox, { transform: [{ translateY: slideAnim }] }]}
      >
        <Animated.View
          style={{
            transform: [
      ...
      <Text style={styles.resultText}>
        {category === "All" ? "All Categories" : category} • {data.length} items
      </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.alertModal}>
            ...
            <Text style={styles.alertText}>
              You've tracked {expenses.length} expense
            ...
            <View style={styles.alertButtons}>
              ...
              <CustomButton
                ...
                onPress={() => {
                  ...
                  Alert.alert("Details", `Total spent: $${total.toFixed(2)}`);
                }}
                color="#81C784"
              />
            </View>
